...
from nuc_detect_modules import data_functions


def train_model( CONVnet, project_specs, cnninput, maskinput, bsize=20, nb_epoch=5, \
                validation_data=False, val_input=None, val_masks=None):
    ...
    history = CONVnet.fit(cnninput, maskinput, batch_size=bsize, epochs=nb_epoch, verbose=1)
...
def domain_adapt( project_specs ):
    ...
    cnninput, maskinput,labels,coordinates = data_functions.get_input_from_confidence(project_specs, \
        block_size=block_size, threshold=TH1, use_multi_threshold=True, threshold2=TH2)
    ...
    for idx in range(len(imagefiles)):
        ...
        for j in range(len(project_specs.output_mode)):
            ...
            if outmode=='coordinates':
                ...
                coordinates = auxiliary_functions.get_coordinates_from_confidence(pred, 0.5, 'int')
                mask = auxiliary_functions.generate_binary_mask_from_coordinates(coordinates, [imsize[0],imsize[1]], 'coordinates')
                io.imsave(fname, mask.astype('ubyte'))
            if outmode=='visualisation':
                fname = outputpath + substr[0] + '_vis.tif'
                
                coordinates = auxiliary_functions.get_coordinates_from_confidence(pred, 0.5, 'int')
                vis_img = auxiliary_functions.get_coordinate_overlay(copy.copy(img), coordinates)
...
def save_training_history( history, savepath, modelname, validation_data=False ):
    ...
    if validation_data:
        traininghistory = {'acc': history.history['acc'], 'loss': history.history['loss'], \
    ...
    now = ''.join([nowsubs[0],'_',nowsubs2[0],'-',nowsubs2[1],'-',nowsubs3[0],'-',nowsubs3[1]])
