...
#from nuc_detect_modules import cnn_functions
from nuc_detect_modules import image_loader 


def get_blocks_from_coordinates( coordinates, input_img, block_size, num_channels, imtype ):
    
    if num_channels==1:
        proc_coord = np.zeros([len(coordinates),2],dtype='int') 
        blocks = np.zeros([len(coordinates),block_size[0],block_size[1],1],dtype=imtype)
        ...
        proc_coord = np.zeros([len(coordinates),2],dtype='int') 
        blocks = np.zeros([len(coordinates),block_size[0],block_size[1],num_channels],dtype=imtype)
        stepx = int((block_size[0])/2)
        stepy = int((block_size[1])/2)
        ...
        input_img = np.zeros((np.shape(input_img[:,:,0])[0]+2*block_size[0],np.shape(input_img[:,:,0])[1]+2*block_size[1],num_channels),dtype=imtype)
        input_img[:,:,0] = R
        input_img[:,:,1] = G
        input_img[:,:,2] = B
...
def augment_data(cnninput, maskinput):
    ...
    amount_added_data = 1
    augmented_cnninput = np.zeros([amount_added_data*datashape[0],\
                    datashape[1],datashape[2],datashape[3]],dtype=datatype)
    augmented_maskinput = np.zeros([amount_added_data*maskshape[0],\
                    maskshape[1],maskshape[2],maskshape[3]],dtype=masktype)
...
def get_input_from_confidence( project_specs, block_size=128, threshold=0.5, use_multi_threshold=False, threshold2=0.6 ):    
    ...
    for idx in range(len(conffiles)):
        ...
        img = image_loader.load_image(imagefile, project_specs.dataset_mpp, project_specs.model_mpp)
        conf = image_loader.load_conf(conffile)
    
        coordinates = auxiliary_functions.get_coordinates_from_confidence(conf, threshold, 'int')
        # use two thresholds, one for mask, one for selecting high confidence nuc
        if use_multi_threshold:
            coordinates2 = auxiliary_functions.get_coordinates_from_confidence(conf, threshold2, 'int')             
        ...
        if len(coordinates2)>0:                      
            mask = auxiliary_functions.generate_binary_mask_from_coordinates(coordinates, np.shape(conf), \
                                            project_specs.masktype, markersize=project_specs.mask_element_size)           
                
            numchannels = 3
            imtype = 'float32'
            tmpcnninput, proc_coord = get_blocks_from_coordinates(coordinates2, img, (block_size,block_size), numchannels, imtype)
            tmpmaskinput, proc_coord_m = get_blocks_from_coordinates(coordinates2, mask, (block_size,block_size), 1, imtype)
